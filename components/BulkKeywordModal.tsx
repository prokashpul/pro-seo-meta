...
import { X, Plus, Minus, RefreshCcw, Trash2 } from 'lucide-react';

export type BulkActionType = 'ADD' | 'REMOVE' | 'REPLACE_ALL' | 'CLEAR_ALL';

interface BulkKeywordModalProps {
  isOpen: boolean;
  onClose: () => void;
  onApply: (action: BulkActionType, keywords: string[]) => void;
...
export const BulkKeywordModal: React.FC<BulkKeywordModalProps> = ({ isOpen, onClose, onApply, selectedCount }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // For REPLACE_ALL we allow empty list (clearing keywords), otherwise we need at least one keyword
    if (action === 'REPLACE_ALL' || keywords.length > 0) { 
      onApply(action, keywords);
      setInputValue('');
      onClose();
    }
  };
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl font-bold text-slate-900 dark:text-white">Bulk Edit Keywords</h3>
          ...
          <div className="flex gap-3">
            ...
            >
              {action === 'CLEAR_ALL' ? 'Clear Keywords' : 'Apply Changes'}
            </button>
          </div>
        </div>
      </div>
